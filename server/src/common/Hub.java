 ...
 * sendToOne(username,msg) method will send the message to just the client with
...
public abstract class Hub {
     ...
     */
    private TreeMap<String, ConnectionToClient> playerConnections;
     ...
     * @throws IOException if it is not possible to create a listening socket on
     * the specified port.
     */
    public Hub(int port) throws IOException {
        playerConnections = new TreeMap<String, ConnectionToClient>();
     ...
     *
     * @param username The ID number of the player who sent the message.
     * @param message The message that was received from the player.
     */
    protected void messageReceived(String username, Object message) {
        sendToAll(new ForwardedMessage(username, message));
    }
     ...
     *
     * @param username the ID number of the new player.
     */
    protected void playerConnected(String username) {
    }
     ...
     * @param username the ID number of the new player.
     */
    protected void playerDisconnected(String username) {
    }
     ...
     * is called after the username has been transmitted to the client. If this
     ...
     *
     * @param username the ID number of the player who is connecting.
     ...
     */
    protected abstract String extraHandshake(ObjectInputStream in,
     ...
     */
    synchronized public String[] getPlayerList() {
        String[] players = new String[playerConnections.size()];
        int i = 0;
        for (String p : playerConnections.keySet()) {
            players[i++] = p;
        }
        return players;
    }
     ...
     * @throws IOException if it is impossible to create a listening socket on
     * the specified port.
     */
     ...
     * object must implement the Serializable interface. Messages must not be
     * null.
     */
     ...
     *
     * @param recipientID The ID number of the player to whom the message is to
     * be sent. If there is no such player, then the method returns the value
     * false.
     * @param message the message to be sent to all connected clients. This
     * object must implement the Serializable interface. Messages must not be
     * null.
     ...
     */
    synchronized public boolean sendToOne(String recipientID, Object message) {
        if (message == null) {
            throw new IllegalArgumentException("Null cannot be sent as a message.");
        }
        if (!(message instanceof Serializable)) {
    ...
    synchronized private void messageReceived(ConnectionToClient fromConnection, Object message) {
        ...
        String sender = fromConnection.getPlayer();
        messageReceived(sender, message);
    }

    synchronized private void acceptConnection(ConnectionToClient newConnection) {
        String username = newConnection.getPlayer();
        playerConnections.put(username, newConnection);
        StatusMessage sm = new StatusMessage(username, true, getPlayerList());
        sendToAll(sm);
        playerConnected(username);
        System.out.println("Connection accepted from " + username);
    }

    synchronized private void clientDisconnected(String username) {
        if (playerConnections.containsKey(username)) {
            playerConnections.remove(username);
            StatusMessage sm = new StatusMessage(username, false, getPlayerList());
            sendToAll(sm);
            playerDisconnected(username);
            System.out.println("Connection with " + username + " closed by DisconnectMessage from client.");
        }
    }

    synchronized private void connectionToClientClosedWithError(ConnectionToClient playerConnection, String message) {
        String username = playerConnection.getPlayer();
        if (playerConnections.remove(username) != null) {
            StatusMessage sm = new StatusMessage(username, false, getPlayerList());
            sendToAll(sm);
        }
        playerDisconnected(username);
        System.out.println("Connection with " + username + " closed by Unknown Error");
    }
    ...
    private class ConnectionToClient { // Handles communication with one client.
        ...
        private String username;  // The ID number for this player.
        ...
        private class SendThread extends Thread {
            ...
            public void run() {
                try {
                    ...
                    if (!"12/09/2013".equals(handle)) {
                        throw new Exception("Incorrect handshake string received from client.");
                    }
                    String authenticated = extraHandshake(in, out);  // Does any extra stuff before connection is fully established.
                    if (!authenticated.equals("invalid")) {
                        ConnectionToClient.this.username = authenticated;
                    } else {
