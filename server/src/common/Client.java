...
abstract public class Client {
     ...
     */
    protected String[] connectedPlayerIDs = new String[0];
     ...
     *
     * @param hubHostName The host name (or IP address) of the computer where
     * the Hub is running.
     ...
     */
    public Client(String hubHostName, int hubPort, String username, String password) throws IOException {
        connection = new ConnectionToHub(hubHostName, hubPort, username, password);
    }
     ...
     * connected to the hub. (Note that is IS called when this client connects,
     ...
     */
    protected void playerConnected(String newPlayerID) {
    }
     ...
     * disconnected.
     */
    protected void playerDisconnected(String departingPlayerID) {
    }
     ...
     */
    protected void extraHandshake(ObjectInputStream in, ObjectOutputStream out, String username, String password)
            throws IOException {
    }
    ...
    private class ConnectionToHub {
        ...
        private final String username;               // The ID of this client, assigned by the hub.
         ...
         */
        ConnectionToHub(String host, int port, String username, String password) throws IOException {
            outgoingMessages = new LinkedBlockingQueue<Object>();
            socket = new Socket(host, port);
            out = new ObjectOutputStream(socket.getOutputStream());
            out.writeObject("12/09/2013");
            out.flush();
            in = new ObjectInputStream(socket.getInputStream());
            extraHandshake(in, out, username, password);  // Will throw an IOException if handshake doesn't succeed.
                ...
                try {
                    while (!closed) {
                        ...
                        if (obj instanceof DisconnectMessage) {
                            ...
                            if (msg.connecting) {
                                playerConnected(msg.username);
                            } else {
                                playerDisconnected(msg.username);
                            }
                        } else {
                            messageReceived(obj);
                        }
